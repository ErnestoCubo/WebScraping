...
cursorObj.execute('''CREATE TABLE EQUIPOS(Equipo TEXT, EdadMedia INT, ValorMedio FLOAT, Actualizacion TEXT)''')
con.commit()


def pasarAdb(resultado,edad,valor,actualizacion):

    con.execute('''INSERT INTO EQUIPOS(Equipo,EdadMedia,ValorMedio,Actualizacion) VALUES(?, ?, ?, ?)''',
                  (resultado,edad,valor,actualizacion))
    con.commit()
...
def scrapeo(url):
    ...
    d = ({"Equipo": resultado, "Edad media": totalEdad, "Valor medio": totalValor, "Actualizacion":actualizacion})
    ...
    pasarAdb(resultado,totalEdad,totalValor,actualizacion)
