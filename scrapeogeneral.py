...
try:
    ...
    cursorObj.execute(
        '''CREATE TABLE EQUIPOS(Equipo TEXT, Jugadores INT, EdadMedia INT, ValorTotal FLOAT, ValorMedio FLOAT, Actualizacion TEXT)''')
    cursorObj.execute(
        '''CREATE TABLE JUGADORES(jugador TEXT, edad INT, pais TEXT, equipos TEXT, dorsal INT, posicion TEXT, precio FLOAT, actualizacion TEXT)''')
...
def pasarAdbMedias(resultado,num,edad,total,valor,actualizacion):
...
def scrapeo_medias(url):
    ...
    pasarAdbMedias(resultado,aux,mediaEdad,totalValor,mediaValor,actualizacion)
